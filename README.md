...
- 针对公司 Wi-Fi 屏蔽 mDNS 或隔离终端的场景，“无线连接”提供公司网络模式：Mac 通过有线/USB 网卡接入公司网络并创建独立 Wi-Fi 热点，iPhone 连接热点后使用普通 AirPlay 投屏；Android USB 不受影响，无线 Android 切换热点后重新连接即可。
...
- 首次启动会显示一次新手引导，介绍文件互传、设备连接、公司网络投屏和截录功能；关闭后不会自动重复出现，可从“帮助”菜单重新打开。
- 启动后每 24 小时最多读取一次 GitHub 官方 `releases/latest` 跳转地址；发现新版本时可打开下载页，也可从“帮助 → 检查更新…”手动检查。该方式不调用 GitHub API、不消耗 API 配额，应用也不会在后台自动安装。
...
开始投屏时直接打开“无线连接 → iPhone AirPlay 投屏（名称与启动）”：在显眼的名称框中修改接收名称，选择内嵌或独立窗口，然后点击“启动 iPhone 接收器”。此入口不依赖 USB iPhone 文件发现。普通模式要求 Mac 与 iPhone 位于可互相发现的局域网；公司 Wi-Fi 隔离终端时，优先使用页面顶部的“公司网络模式”，由 Mac 创建热点后保持附近模式关闭。没有共同 Wi-Fi 时也可尝试附近设备投屏（AWDL）并输入界面 PIN，该模式依赖 Mac 与 iPhone 的 Wi-Fi/蓝牙，不是纯 USB 视频通道。
